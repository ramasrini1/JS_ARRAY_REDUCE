...
function extractValue(arr, key) {
    return arr.reduce(function(acc, nextItem) {
        acc.push(nextItem[key]);
        return acc;
    }, []);
}
...
with the keys as the vowel and the values as the number of times the 
...
function vowelCount(str) {
    ...
    arr = strLower.split('');
    let vowels = "aeiou";
    return arr.reduce(function(vowelFreq, next) {
...
The partition function should run the callback function on each value in 
the array and if the result of the callback function at that specific value is true, 
